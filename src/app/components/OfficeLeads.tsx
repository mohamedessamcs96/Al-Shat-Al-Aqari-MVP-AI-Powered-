...
import { ArrowRight, Phone, Mail, MessageSquare, Filter, Search, Clock } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <Tabs defaultValue="all">
          ...
          <TabsContent value="new" className="mt-6">
            <div className="space-y-4">
              {filteredLeads.filter(l => l.validation_status === 'pending').map((lead) => (
                <Card key={lead.id} className="p-6">
                  <div className="flex items-start justify-between" dir="rtl">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div><p className="text-sm text-gray-600">نوع العقار</p><p className="font-medium text-gray-900">{lead.property_type}</p></div>
                        <div><p className="text-sm text-gray-600">المدينة</p><p className="font-medium text-gray-900">{getCityName(lead.city_id)}</p></div>
                        <div><p className="text-sm text-gray-600">الميزانية</p><p className="font-medium text-gray-900">{formatPrice(lead.budget_min)} - {formatPrice(lead.budget_max)}</p></div>
                        <div><p className="text-sm text-gray-600">الحد الأدنى للغرف</p><p className="font-medium text-gray-900">{lead.bedrooms_min} غرف</p></div>
                      </div>
                      {lead.notes && <div className="bg-blue-50 p-3 rounded-lg mb-4"><p className="text-sm text-gray-700">{lead.notes}</p></div>}
                      <div className="flex items-center gap-2 text-sm text-gray-500"><Clock className="w-4 h-4" /><span>{new Date(lead.created_at).toLocaleDateString('ar-SA')}</span></div>
                    </div>
                    <div className="flex flex-col gap-2 mr-4">
                      <Dialog open={selectedLead === lead.id} onOpenChange={(open) => !open && setSelectedLead(null)}>
                        <DialogTrigger asChild>
                          <Button size="sm" onClick={() => setSelectedLead(lead.id)}><MessageSquare className="w-4 h-4 ml-2" />الرد</Button>
                        </DialogTrigger>
                        <DialogContent dir="rtl">
                          <DialogHeader><DialogTitle>الرد على {lead.buyer_name}</DialogTitle></DialogHeader>
                          <div className="space-y-4 mt-4">
                            <div className="bg-gray-50 p-3 rounded-lg">
                              <p className="text-sm text-gray-700">يبحث عن {lead.property_type} في {getCityName(lead.city_id)}</p>
                              <p className="text-sm text-gray-700 mt-1">الميزانية: {formatPrice(lead.budget_min)} - {formatPrice(lead.budget_max)}</p>
                            </div>
                            <div><Label htmlFor="response-message-new">رسالتك</Label><Textarea id="response-message-new" value={responseMessage} onChange={(e) => setResponseMessage(e.target.value)} placeholder="مرحباً، لدينا عدة عقارات تناسب متطلباتك..." rows={6} className="mt-1" /></div>
                            <div className="flex gap-2"><Button onClick={handleRespond} className="flex-1">إرسال</Button><Button variant="outline" className="flex-1" onClick={() => setSelectedLead(null)}>إلغاء</Button></div>
                          </div>
                        </DialogContent>
                      </Dialog>
                      <Button size="sm" variant="outline"><Phone className="w-4 h-4 ml-2" />اتصال</Button>
                      <Button size="sm" variant="outline"><Mail className="w-4 h-4 ml-2" />بريد</Button>
                    </div>
                  </div>
                </Card>
              ))}
              {filteredLeads.filter(l => l.validation_status === 'pending').length === 0 && (
                <Card className="p-12 text-center"><MessageSquare className="w-12 h-12 text-gray-400 mx-auto mb-3" /><h3 className="font-semibold text-gray-900 mb-2" dir="rtl">لا توجد عملاء جدد</h3></Card>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="urgent" className="mt-6">
            <div className="space-y-4">
              {filteredLeads.filter(l => l.intent_level === 'urgent').map((lead) => (
                <Card key={lead.id} className="p-6">
                  <div className="flex items-start justify-between" dir="rtl">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                      ...
                      {lead.notes && <div className="bg-red-50 p-3 rounded-lg mb-4"><p className="text-sm text-gray-700">{lead.notes}</p></div>}
                      <div className="flex items-center gap-2 text-sm text-gray-500"><Clock className="w-4 h-4" /><span>{new Date(lead.created_at).toLocaleDateString('ar-SA')}</span></div>
                    </div>
                    <div className="flex flex-col gap-2 mr-4">
                      <Dialog open={selectedLead === lead.id} onOpenChange={(open) => !open && setSelectedLead(null)}>
                        ...
                        <DialogContent dir="rtl">
                          ...
                          <div className="space-y-4 mt-4">
                            ...
                            <div><Label htmlFor="response-message-urgent">رسالتك</Label><Textarea id="response-message-urgent" value={responseMessage} onChange={(e) => setResponseMessage(e.target.value)} placeholder="مرحباً، لدينا عدة عقارات تناسب متطلباتك..." rows={6} className="mt-1" /></div>
                            <div className="flex gap-2"><Button onClick={handleRespond} className="flex-1">إرسال</Button><Button variant="outline" className="flex-1" onClick={() => setSelectedLead(null)}>إلغاء</Button></div>
                          </div>
                        </DialogContent>
                      </Dialog>
              ...
              {filteredLeads.filter(l => l.intent_level === 'urgent').length === 0 && (
                <Card className="p-12 text-center"><MessageSquare className="w-12 h-12 text-gray-400 mx-auto mb-3" /><h3 className="font-semibold text-gray-900 mb-2" dir="rtl">لا توجد عملاء عاجلون</h3></Card>
              )}
            </div>
          </TabsContent>
